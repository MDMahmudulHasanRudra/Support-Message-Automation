...
import {
  deleteTeamsResolutionKeyword,
  toggleTeamsResolutionKeywordActive,
  updateTeamsResolutionKeyword,
} from "@/server/actions/teamsResolutionKeywords";

export interface TeamsResolutionKeywordRow {
  id: string;
  value: string;
  matchMode: string;
...
type ConfirmState = { kind: "delete" | "toggle"; keyword: TeamsResolutionKeywordRow } | null;

export function TeamsResolutionKeywordsTable({ keywords }: { keywords: TeamsResolutionKeywordRow[] }) {
  const router = useRouter();
  const [confirmState, setConfirmState] = useState<ConfirmState>(null);
  const [editing, setEditing] = useState<TeamsResolutionKeywordRow | null>(null);
  ...
  function confirmAction() {
    ...
    startTransition(async () => {
      if (kind === "delete") await deleteTeamsResolutionKeyword(keyword.id);
      if (kind === "toggle") await toggleTeamsResolutionKeywordActive(keyword.id);
  ...
  async function saveEdit(formData: FormData) {
    if (!editing) return;
    await updateTeamsResolutionKeyword(editing.id, formData);
    setEditing(null);
    router.refresh();
  }

  if (keywords.length === 0) {
    return <EmptyState>No resolution keywords yet — add one above.</EmptyState>;
  }
  ...
  return (
    <div>
      ...
      <ConfirmDialog
        ...
        description={confirmState?.kind === "delete" ? "This also removes it from any resolution rule that references it." : undefined}
